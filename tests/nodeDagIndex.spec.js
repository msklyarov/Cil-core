...
const createSimpleChain = async (callback) => {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (prevBlock) {
            ...
            console.log('GEN', factory.Constants.GENESIS_BLOCK)
        }
        prevBlock = block;
...
describe('Node tests', async () => {
    ...
    it('should build EMPTY MainDag (no blocks were processed-final)', async () => {
        ...
        //assert.equal(await node._mainDag.order(), 0);
    ...
    it('should build MainDag from chainlike', async () => {
        ...
        console.log('AAAAAAAAAAAAAAAaaa', arrPendingHashes)

        await node._buildMainDagIndex([], arrPendingHashes);

        // assert.equal(await node._mainDag.order(), 10);
    ...
    it('should build MainDag from simple fork', async () => {
        ...
        console.log('AAAAAAAA', arrBlocks)
        ...
        // assert.deepEqual(
        //     node._mainDagIndex.getParents(arrBlocks[3].getHash()),
        //     [arrBlocks[1].getHash(), arrBlocks[2].getHash()]
        // );
        assert.deepEqual(
            await node._mainDagIndex.getChildren(arrBlocks[0].getHash()),
            [arrBlocks[1].getHash(), arrBlocks[2].getHash()]
        );
    });
    ...
    describe('Node bootstrap', async () => {
        ...
        describe('Node with gap more than MAX_BLOCKS_INV', async () => {
            ...
            it('should return just beginning of chain (except attached to root)', async () => {
                ...
                const setResult = await node._getBlocksFromLastKnown2([arrExpectedHashes[nPosInChain]]);


                console.log('RRRR', setResult)
            ...
            it('should get all', async () => {
                ...
                const setResult = await node._getBlocksFromLastKnown2([arrExpectedHashes[0]]);
        ...
        describe('Node with side chain gap more than MAX_BLOCKS_INV', async () => {
            ...
            it('should return beginning of chain and 1st block of side', async () => {
                ...
                const setResult = await node._getBlocksFromLastKnown2([arrMainChainHashes[nPosInChain]]);
            ...
            it('should get all', async () => {
                ...
                const setResult = await node._getBlocksFromLastKnown2([arrMainChainHashes[0]]);
